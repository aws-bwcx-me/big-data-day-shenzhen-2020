...
if [ $# != 2 ]; then
    ...
    echo "Example: $0 kds-lab1 2020-09-04"
    exit 1
fi
...
# write log
function write_log() {
    echo "$(date +%FT%TZ) ## $0 ## $1" >>./logs-lab1-$(date +%F).log
}
...
while true; do
    ...
    while [[ $i -le ${t_thistime} ]]; do
        ...
        write_log "data: $data"
        aws kinesis put-record --stream-name ${stream_name} --data $data --partition-key ${get_tno} --region ${region_name} >>./logs-lab1-$(date +%F).log 2>&1
        beg_tid=${beg_tid}+1
        i=$i+1
    done
    # random slepp
